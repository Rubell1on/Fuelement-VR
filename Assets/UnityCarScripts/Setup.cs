...
public class Setup : MonoBehaviour {
	...
	void SaveBodyData(){
		SaveValue("weight",GetComponent<Rigidbody>().mass+"","body");
		...
		else SaveValue("centerOfMass",GetComponent<Rigidbody>().centerOfMass+"","body");
	...
	void LoadBodyData(){
        ...
        if (value!="") GetComponent<Rigidbody>().mass=float.Parse(value);
		value=LoadValue("weightRepartition","body");
        if (value!="") cardynamics.frontRearWeightRepartition=float.Parse(value);
		value=LoadValue("centerOfMass","body"); if (value!="") {
			value=value.Replace("(","").Replace(")","");
            string[] temp = new List<string>(value.Split(','))
                .Select(s => s.Trim().Replace('.', ','))
                .ToArray();

            float value0=float.Parse(temp[0]);
			float value1=float.Parse(temp[1]);
			float value2=float.Parse(temp[2]);
        ...
        if (value!="") cardynamics.inertiaFactor=float.Parse(value);
		if (aerodynamicResistance!=null) {
            value =LoadValue("dragCoefficent","body");
            if (value!="") aerodynamicResistance.Cx=float.Parse(value);
        }
		if (aerodynamicResistance!=null) {
            value =LoadValue("dragArea","body");
            if (value!="") aerodynamicResistance.Area=float.Parse(value);
        }
	}	
	...
	void LoadEngineData(){
		...
		value=LoadValue("minRPM","engine"); if (value!="") drivetrain.minRPM=float.Parse(value);
		value=LoadValue("maxRPM","engine"); if (value!="") drivetrain.maxRPM=float.Parse(value);		
		...
		if (dataPairs!=""){
			...
			foreach (string line in dataLines)
			{
				drivetrain.torqueRPMValues[lineNum,0] = float.Parse(line.Split(',')[0]);
				drivetrain.torqueRPMValues[lineNum,1] = float.Parse(line.Split(',')[1]);
				lineNum++;
			}
		...
		if (drivetrain.engineTorqueFromFile==false){ // these values are calculated from the torque file
			value=LoadValue("maxPower","engine"); if (value!="") drivetrain.maxPower=float.Parse(value);
			value=LoadValue("maxPowerRPM","engine"); if (value!="") drivetrain.maxPowerRPM=float.Parse(value);
			value=LoadValue("maxTorque","engine"); if (value!="") drivetrain.maxTorque=float.Parse(value);
			value=LoadValue("maxTorqueRPM","engine"); if (value!="") drivetrain.maxTorqueRPM=float.Parse(value);
		}				
		...
		value=LoadValue("revLimiterTime","engine"); if (value!="") drivetrain.revLimiterTime=float.Parse(value);
		value=LoadValue("engineInertia","engine"); if (value!="") drivetrain.engineInertia=float.Parse(value);
		value=LoadValue("engineFrictionFactor","engine"); if (value!="") drivetrain.engineFrictionFactor=float.Parse(value);
		...
		value=LoadValue("canStall","engine");	if (value!="") drivetrain.canStall=bool.Parse(value);
		value=LoadValue("fuelConsumptionAtCostantSpeed","engine"); if (value!="") drivetrain.fuelConsumptionAtCostantSpeed=float.Parse(value);
		value=LoadValue("fuelConsumptionSpeed","engine"); if (value!="") drivetrain.fuelConsumptionSpeed=float.Parse(value);
	...
	void LoadTransmissionData(){
		...
		if(drivetrain.gearRatios.Length>0) {value=LoadValue("gear-ratio-r","transmission"); if (value!="") drivetrain.gearRatios[0]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>2) {value=LoadValue("gear-ratio-1","transmission"); if (value!="") drivetrain.gearRatios[2]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>3) {value=LoadValue("gear-ratio-2","transmission"); if (value!="") drivetrain.gearRatios[3]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>4) {value=LoadValue("gear-ratio-3","transmission"); if (value!="") drivetrain.gearRatios[4]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>5) {value=LoadValue("gear-ratio-4","transmission"); if (value!="") drivetrain.gearRatios[5]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>6) {value=LoadValue("gear-ratio-5","transmission"); if (value!="") drivetrain.gearRatios[6]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>7) {value=LoadValue("gear-ratio-6","transmission"); if (value!="") drivetrain.gearRatios[7]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>8) {value=LoadValue("gear-ratio-7","transmission"); if (value!="") drivetrain.gearRatios[8]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>9) {value=LoadValue("gear-ratio-8","transmission"); if (value!="") drivetrain.gearRatios[9]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>10) {value=LoadValue("gear-ratio-9","transmission"); if (value!="") drivetrain.gearRatios[10]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>11) {value=LoadValue("gear-ratio-10","transmission"); if (value!="") drivetrain.gearRatios[11]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>12) {value=LoadValue("gear-ratio-11","transmission"); if (value!="") drivetrain.gearRatios[12]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>13) {value=LoadValue("gear-ratio-12","transmission"); if (value!="") drivetrain.gearRatios[13]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>14) {value=LoadValue("gear-ratio-13","transmission"); if (value!="") drivetrain.gearRatios[14]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>15) {value=LoadValue("gear-ratio-14","transmission"); if (value!="") drivetrain.gearRatios[15]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>16) {value=LoadValue("gear-ratio-15","transmission"); if (value!="") drivetrain.gearRatios[16]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>17) {value=LoadValue("gear-ratio-16","transmission"); if (value!="") drivetrain.gearRatios[17]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>18) {value=LoadValue("gear-ratio-17","transmission"); if (value!="") drivetrain.gearRatios[18]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>19) {value=LoadValue("gear-ratio-18","transmission"); if (value!="") drivetrain.gearRatios[19]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>20) {value=LoadValue("gear-ratio-19","transmission"); if (value!="") drivetrain.gearRatios[20]=float.Parse(value);}
		if(drivetrain.gearRatios.Length>21) {value=LoadValue("gear-ratio-20","transmission"); if (value!="") drivetrain.gearRatios[21]=float.Parse(value);}
		value=LoadValue("finalDriveRatio","transmission"); if (value!="") drivetrain.finalDriveRatio=float.Parse(value);
		value=LoadValue("drivetrainInertia","transmission"); if (value!="") drivetrain.drivetrainInertia=float.Parse(value);		
		value=LoadValue("differentialLockCoefficient","transmission"); if (value!="") drivetrain.differentialLockCoefficient=float.Parse(value);			
		...
		value=LoadValue("shiftDownRPM","transmission"); if (value!="") drivetrain.shiftDownRPM=float.Parse(value);
		value=LoadValue("shiftUpRPM","transmission"); if (value!="" && value!="0") drivetrain.shiftUpRPM=float.Parse(value);
		value=LoadValue("shiftTime","transmission"); if (value!="") drivetrain.shiftTime=float.Parse(value);
		value=LoadValue("clutchMaxTorque","transmission"); if (value!="") drivetrain.clutchMaxTorque=float.Parse(value);
		value=LoadValue("autoClutch","transmission"); if (value!="") drivetrain.autoClutch=bool.Parse(value);
		value=LoadValue("engageRPM","transmission"); if (value!="") drivetrain.engageRPM=float.Parse(value);
		value=LoadValue("disengageRPM","transmission"); if (value!="") drivetrain.disengageRPM=float.Parse(value);
	}

	void LoadSuspensionsData(){
		value=LoadValue("suspensionTravel","suspensions-frontAxle"); if (value!="") axles.frontAxle.suspensionTravel=float.Parse(value);
		value=LoadValue("suspensionRate","suspensions-frontAxle"); if (value!="") axles.frontAxle.suspensionRate=float.Parse(value);
		value=LoadValue("bumpRate","suspensions-frontAxle"); if (value!="") axles.frontAxle.bumpRate=float.Parse(value);
		value=LoadValue("reboundRate","suspensions-frontAxle"); if (value!="") axles.frontAxle.reboundRate=float.Parse(value);
		value=LoadValue("fastBumpFactor","suspensions-frontAxle"); if (value!="") axles.frontAxle.fastBumpFactor=float.Parse(value);
		value=LoadValue("fastReboundFactor","suspensions-frontAxle"); if (value!="") axles.frontAxle.fastReboundFactor=float.Parse(value);
		value=LoadValue("camber","suspensions-frontAxle"); if (value!="") axles.frontAxle.camber=float.Parse(value);
		value=LoadValue("antiRollBarRate","suspensions-frontAxle"); if (value!="") axles.frontAxle.antiRollBarRate=float.Parse(value);
		value=LoadValue("maxSteeringAngle","suspensions-frontAxle"); if (value!="") axles.frontAxle.maxSteeringAngle=float.Parse(value);
		
		value=LoadValue("suspensionTravel","suspensions-rearAxle"); if (value!="") axles.rearAxle.suspensionTravel=float.Parse(value);
		value=LoadValue("suspensionRate","suspensions-rearAxle"); if (value!="") axles.rearAxle.suspensionRate=float.Parse(value);
		value=LoadValue("bumpRate","suspensions-rearAxle"); if (value!="") axles.rearAxle.bumpRate=float.Parse(value);
		value=LoadValue("reboundRate","suspensions-rearAxle"); if (value!="") axles.rearAxle.reboundRate=float.Parse(value);
		value=LoadValue("fastBumpFactor","suspensions-rearAxle"); if (value!="") axles.rearAxle.fastBumpFactor=float.Parse(value);
		value=LoadValue("fastReboundFactor","suspensions-rearAxle"); if (value!="") axles.rearAxle.fastReboundFactor=float.Parse(value);
		value=LoadValue("camber","suspensions-rearAxle"); if (value!="") axles.rearAxle.camber=float.Parse(value);
		value=LoadValue("antiRollBarRate","suspensions-rearAxle"); if (value!="") axles.rearAxle.antiRollBarRate=float.Parse(value);
		value=LoadValue("maxSteeringAngle","suspensions-rearAxle"); if (value!="") axles.rearAxle.maxSteeringAngle=float.Parse(value);

		i=0;
		foreach(Axle axle in axles.otherAxles){
			i++;		
			value=LoadValue("suspensionTravel","suspensions-otherAxle"+i); if (value!="") axle.suspensionTravel=float.Parse(value);
			value=LoadValue("suspensionRate","suspensions-otherAxle"+i); if (value!="") axle.suspensionRate=float.Parse(value);
			value=LoadValue("bumpRate","suspensions-otherAxle"+i); if (value!="") axle.bumpRate=float.Parse(value);
			value=LoadValue("reboundRate","suspensions-otherAxle"+i); if (value!="") axle.reboundRate=float.Parse(value);
			value=LoadValue("fastBumpFactor","suspensions-otherAxle"+i); if (value!="") axle.fastBumpFactor=float.Parse(value);
			value=LoadValue("fastReboundFactor","suspensions-otherAxle"+i); if (value!="") axle.fastReboundFactor=float.Parse(value);
			value=LoadValue("camber","suspensions-otherAxle"+i); if (value!="") axle.camber=float.Parse(value);
			value=LoadValue("antiRollBarRate","suspensions-otherAxle"+i); if (value!="") axle.antiRollBarRate=float.Parse(value);
			value=LoadValue("maxSteeringAngle","suspensions-otherAxle"+i); if (value!="") axle.maxSteeringAngle=float.Parse(value);
		}
	}
	
	void LoadBrakesData(){
		value=LoadValue("brakeFrictionTorque","brakes-frontAxle"); if (value!="") axles.frontAxle.brakeFrictionTorque=float.Parse(value);
		value=LoadValue("handbrakeFrictionTorque","brakes-frontAxle"); if (value!="") axles.frontAxle.handbrakeFrictionTorque=float.Parse(value);
		
		value=LoadValue("brakeFrictionTorque","brakes-rearAxle"); if (value!="") axles.rearAxle.brakeFrictionTorque=float.Parse(value);		
		value=LoadValue("handbrakeFrictionTorque","brakes-rearAxle"); if (value!="") axles.rearAxle.handbrakeFrictionTorque=float.Parse(value);
		
		i=0;
		foreach(Axle axle in axles.otherAxles){
			i++;
			value=LoadValue("brakeFrictionTorque","brakes-otherAxle"+i); if (value!="") axle.brakeFrictionTorque=float.Parse(value);
			value=LoadValue("handbrakeFrictionTorque","brakesotherAxle"+i); if (value!="") axle.handbrakeFrictionTorque=float.Parse(value);
		}

		value=LoadValue("frontRearBrakeBalance","brakes"); if (value!="") cardynamics.frontRearBrakeBalance=float.Parse(value);
		value=LoadValue("frontRearHandBrakeBalance","brakes"); if (value!="") cardynamics.frontRearHandBrakeBalance=float.Parse(value);
	}		

	void LoadTiresData(){
		value=LoadValue("tireType","tires-frontAxle").ToLower(); if (value!="") LoadTireType(value, axles.frontAxle);
		value=LoadValue("forwardGripFactor","tires-frontAxle"); if (value!="") axles.frontAxle.forwardGripFactor=float.Parse(value);
		value=LoadValue("sidewaysGripFactor","tires-frontAxle"); if (value!="") axles.frontAxle.sidewaysGripFactor=float.Parse(value);
		value=LoadValue("tiresPressure","tires-frontAxle"); if (value!="") axles.frontAxle.tiresPressure=float.Parse(value);
		value=LoadValue("optimalTiresPressure","tires-frontAxle"); if (value!="") axles.frontAxle.optimalTiresPressure=float.Parse(value);
		
		value=LoadValue("tireType","tires-rearAxle").ToLower(); if (value!="") LoadTireType(value, axles.rearAxle);
		value=LoadValue("forwardGripFactor","tires-rearAxle"); if (value!="") axles.rearAxle.forwardGripFactor=float.Parse(value);
		value=LoadValue("sidewaysGripFactor","tires-rearAxle"); if (value!="") axles.rearAxle.sidewaysGripFactor=float.Parse(value);
		value=LoadValue("tiresPressure","tires-rearAxle"); if (value!="") axles.rearAxle.tiresPressure=float.Parse(value);
		value=LoadValue("optimalTiresPressure","tires-rearAxle"); if (value!="") axles.rearAxle.optimalTiresPressure=float.Parse(value);

		i=0;
		foreach(Axle axle in axles.otherAxles){
			i++;
			value=LoadValue("tireType","tires-otherAxle"+i).ToLower(); if (value!="") LoadTireType(value, axle);
			value=LoadValue("forwardGripFactor","tires-otherAxle"+i); if (value!="") axle.forwardGripFactor=float.Parse(value);
			value=LoadValue("sidewaysGripFactor","tires-otherAxle"+i); if (value!="") axle.sidewaysGripFactor=float.Parse(value);
			value=LoadValue("tiresPressure","tires-otherAxle"+i); if (value!="") axle.tiresPressure=float.Parse(value);
			value=LoadValue("optimalTiresPressure","tires-otherAxle"+i); if (value!="") axle.optimalTiresPressure=float.Parse(value);
		}		
 	}
	...
	void LoadWheelData(Wheel w, string wheelPosition){
		value=LoadValue("mass","wheels-"+wheelPosition); if (value!="") w.mass=float.Parse(value);
		value=LoadValue("radius","wheels-"+wheelPosition); if (value!="") w.radius=float.Parse(value);
		value=LoadValue("rimRadius","wheels-"+wheelPosition); if (value!="") w.rimRadius=float.Parse(value);
		value=LoadValue("width","wheels-"+wheelPosition); if (value!="") w.width=float.Parse(value);
	}	
	...
	void LoadWingsData(){
		...
		foreach(Wing wing in wings){
			i++;
			value=LoadValue("dragCoefficient", "wing"+i); if (value!="") wing.dragCoefficient=float.Parse(value);
			value=LoadValue("angleOfAttack","wing"+i); if (value!="") wing.angleOfAttack=int.Parse(value);
			value=LoadValue("area","wing"+i); if (value!="") wing.area=float.Parse(value);
		}
	}		
	...
	void LoadControllerData(CarController carController, CarDynamics.Controller controller){
		...
		value=LoadValue("throttleTime",controller+"Controller"); if (value!="") carController.throttleTime =float.Parse(value);
		value=LoadValue("throttleReleaseTime",controller+"Controller"); if (value!="") carController.throttleReleaseTime =float.Parse(value);
		value=LoadValue("maxThrottleInReverse",controller+"Controller"); if (value!="") carController.maxThrottleInReverse=float.Parse(value);
		value=LoadValue("brakesTime",controller+"Controller"); if (value!="") carController.brakesTime =float.Parse(value);
		value=LoadValue("brakesReleaseTime",controller+"Controller"); if (value!="") carController.brakesReleaseTime =float.Parse(value);
		value=LoadValue("steerTime",controller+"Controller"); if (value!="") carController.steerTime =float.Parse(value);
		value=LoadValue("steerReleaseTime",controller+"Controller"); if (value!="") carController.steerReleaseTime =float.Parse(value);
		value=LoadValue("veloSteerTime",controller+"Controller"); if (value!="") carController.veloSteerTime=float.Parse(value);
		value=LoadValue("veloSteerReleaseTime",controller+"Controller"); if (value!="") carController.veloSteerReleaseTime =float.Parse(value);
		value=LoadValue("steerCorrectionFactor",controller+"Controller"); if (value!="") carController.steerCorrectionFactor =float.Parse(value);
		value=LoadValue("steerAssistance",controller+"Controller"); if (value!="") carController.steerAssistance=bool.Parse(value);
		value=LoadValue("SteerAssistanceMinVelocity",controller+"Controller"); if (value!="") carController.SteerAssistanceMinVelocity=float.Parse(value);
		value=LoadValue("TCS",controller+"Controller"); if (value!="") carController.TCS=bool.Parse(value);
		value=LoadValue("TCSThreshold",controller+"Controller"); if (value!="") carController.TCSThreshold=float.Parse(value);
		value=LoadValue("TCSMinVelocity",controller+"Controller"); if (value!="") carController.TCSMinVelocity=float.Parse(value);
		value=LoadValue("ABS",controller+"Controller"); if (value!="") carController.ABS=bool.Parse(value);
		value=LoadValue("ABSThreshold",controller+"Controller"); if (value!="") carController.ABSThreshold=float.Parse(value);
		value=LoadValue("ABSMinVelocity",controller+"Controller"); if (value!="") carController.ABSMinVelocity=float.Parse(value);
		value=LoadValue("ESP",controller+"Controller"); if (value!="") carController.ESP=bool.Parse(value);
		value=LoadValue("ESPStrength",controller+"Controller"); if (value!="") carController.ESPStrength=float.Parse(value);
		value=LoadValue("ESPMinVelocity",controller+"Controller"); if (value!="") carController.ESPMinVelocity=float.Parse(value);	
	}
	
	void LoadPhysicMaterialsData(){
		if (physicMaterials!=null){
			value=LoadValue("trackGrip", "physicMaterials"); if (value!="") physicMaterials.trackGrip=float.Parse(value);
			value=LoadValue("trackRollingFriction", "physicMaterials"); if (value!="") physicMaterials.trackRollingFriction=float.Parse(value);
			value=LoadValue("trackStaticFriction", "physicMaterials"); if (value!="") physicMaterials.trackStaticFriction=float.Parse(value);
			value=LoadValue("grassGrip", "physicMaterials"); if (value!="") physicMaterials.grassGrip=float.Parse(value);
			value=LoadValue("grassRollingFriction", "physicMaterials"); if (value!="") physicMaterials.grassRollingFriction=float.Parse(value);
			value=LoadValue("grassStaticFriction", "physicMaterials"); if (value!="") physicMaterials.grassStaticFriction=float.Parse(value);
			value=LoadValue("sandGrip", "physicMaterials"); if (value!="") physicMaterials.sandGrip=float.Parse(value);
			value=LoadValue("sandRollingFriction", "physicMaterials"); if (value!="") physicMaterials.sandRollingFriction=float.Parse(value);
			value=LoadValue("sandStaticFriction", "physicMaterials"); if (value!="") physicMaterials.sandStaticFriction=float.Parse(value);
			value=LoadValue("offRoadGrip", "physicMaterials"); if (value!="") physicMaterials.offRoadGrip=float.Parse(value);
			value=LoadValue("offRoadRollingFriction", "physicMaterials"); if (value!="") physicMaterials.offRoadRollingFriction=float.Parse(value);
			value=LoadValue("offRoadStaticFriction", "physicMaterials"); if (value!="") physicMaterials.offRoadStaticFriction=float.Parse(value);
		}
	}		

	void LoadArcaderData(){
		if (arcader!=null) {
			value=LoadValue("minVelocity", "arcader"); if (value!="") arcader.minVelocity=float.Parse(value);
			value=LoadValue("overallStrength", "arcader"); if (value!="") arcader.overallStrength=float.Parse(value);
			value=LoadValue("COGHelperStrength", "arcader"); if (value!="") arcader.COGHelperStrength=float.Parse(value);
			value=LoadValue("torqueHelperStrength", "arcader"); if (value!="") arcader.torqueHelperStrength=float.Parse(value);
			value=LoadValue("gripHelperStrength", "arcader"); if (value!="") arcader.gripHelperStrength=float.Parse(value);
	...
	void LoadFuelTanksData(){
		...
		foreach(FuelTank fuelTank in fuelTanks){
			i++;
			value=LoadValue("tankCapacity","fuelTank"+i); if (value!="") fuelTank.tankCapacity=float.Parse(value);
			value=LoadValue("currentFuel", "fuelTank"+i); if (value!="") fuelTank.currentFuel=float.Parse(value);
			value=LoadValue("tankWeight", "fuelTank"+i); if (value!="") fuelTank.tankWeight=float.Parse(value);
			value=LoadValue("fuelDensity","fuelTank"+i); if (value!="") fuelTank.fuelDensity=float.Parse(value);
		}
	}	
	...
	void LoadForceFeedBackData(){
		if (forceFeedback!=null) {
			...
			value=LoadValue("multiplier", "forcefeedback"); if (value!="") forceFeedback.multiplier=float.Parse(value);
			value=LoadValue("smoothingFactor", "forcefeedback"); if (value!="") forceFeedback.smoothingFactor=float.Parse(value);
