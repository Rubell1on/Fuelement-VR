...
public class CarDynamics : MonoBehaviour {
	...
	public float GetCentrifugalAccel(){
		...
		return val*invAllWheelsLength/GetComponent<Rigidbody>().mass; // doesnt work with variable body
	}
	...
	public void SetCenterOfMass(Vector3? COGPosition=null){
		if (centerOfMass == null){
			...
			if (COGPosition!=null) GetComponent<Rigidbody>().centerOfMass=centerOfMass.localPosition=COGPosition.Value;
			else centerOfMass.localPosition=GetComponent<Rigidbody>().centerOfMass;
		}
		else{
			if (COGPosition!=null) GetComponent<Rigidbody>().centerOfMass=centerOfMass.localPosition=COGPosition.Value;
			else GetComponent<Rigidbody>().centerOfMass=centerOfMass.localPosition;
