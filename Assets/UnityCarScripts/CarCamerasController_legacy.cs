...
public class CarCamerasController_legacy : MonoBehaviour 
{
	...
	public void SetCamera(int index, Transform target, bool newTarget){
		...
		if (index==0 &&  target!=null){ //external
			carCameras.mycamera=CustomCamera.CameraType.SmoothLookAt;
			...
			carCameras.mycamera=CustomCamera.CameraType.FixedTo;
			...
			carCameras.mycamera=CustomCamera.CameraType.MouseOrbit;
			carCameras.target=target;
			...
			carCameras.mycamera=CustomCamera.CameraType.Map;
			carCameras.target=target;
			carCameras.distance=0;
			carCameras.height=0;
			carCameras.pitchAngle=0;
	...
	void Update(){
		//Camera control
		if (carCameras.mycamera!=CustomCamera.CameraType.Map){
			if (Input.GetKeyDown (KeyCode.C)) {
				presetId+=1;
				if (presetId==7) presetId=0;
				SetCamera(presetId, carCameras.mtarget, false);
			}
		}
	}
}