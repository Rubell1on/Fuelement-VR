 ...
 */		dashBoard=car.GetComponentInChildren<DashBoard>();
		if (dashBoard!=null) dashBoard.gameObject.active=false;
		if (car.transform.GetComponent<CarDebug>()!=null) car.transform.GetComponent<CarDebug>().enabled=false;
...
/* 		if (Time.fixedDeltaTime<=0.02f) {
		...
		dashBoard=car.GetComponentInChildren<DashBoard>();
		if (dashBoard!=null) dashBoard.gameObject.active=true;
	...
	void Update(){
		...
		if (Input.GetKeyUp (KeyCode.M)){
			if (mapCameraController	!=null) mapCameraController.gameObject.active=!mapCameraController.gameObject.active;
		}
